...
public class ClapsView: UIView {
    ...
    fileprivate let timerDuration: TimeInterval = 0.10
    ...
    internal func finalizeClaps() {
        ...
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + timerDuration, execute: {
            ...
            if self.bubbleTimer == nil {
                self.removeBubbleForTimerNil()
            }
            else {
                self.animateBubbleForContinousTap()
            }
    ...
    func animateBubbleForContinousTap(){
        UIView.animate(withDuration: self.animationDuration, delay: 0.0, options: .curveEaseOut, animations: {
            self.clapsLabel?.alpha = 1.0
            self.clapsLabel?.transform = CGAffineTransform(scaleX: 1.2,y: 1.2)
        }) { (isCompleted) in
            self.clapsLabel?.alpha = 1.0
            self.clapsLabel?.transform = CGAffineTransform(scaleX: 1.6,y: 1.6)
        }
    ...
    func setBubbleTimer() {
        if bubbleTimer == nil {
            bubbleTimer = Timer.scheduledTimer(timeInterval: 2.0, target: self, selector: #selector(stopAnimation), userInfo: nil, repeats: false)
        } else {
